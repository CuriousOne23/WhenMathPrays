# Documentation Contents

This document provides a comprehensive index of all documentation in the WhenMathPrays project, organized by category and purpose.

## Table of Contents

- [Core Documentation](#core-documentation)
- [Architecture & Design](#architecture--design)
- [User Guides](#user-guides)
- [Development & Testing](#development--testing)
- [Research & Theory](#research--theory)
- [Configuration & Setup](#configuration--setup)
- [Legacy & Archive](#legacy--archive)
- [Navigation Guide](#navigation-guide)

## Core Documentation

### Project Overview
- **[README.md](../README.md)** - Main project README with overview, installation, and usage
- **[WHY_THIS_MATTERS.md](../WHY_THIS_MATTERS.md)** - Philosophical and practical importance of the GRP framework
- **[grp_principles.md](grp_principles.md)** - Core principles and foundational concepts
- **[LINEAGE.md](LINEAGE.md)** - Historical development and evolution of the project

### Constants & Configuration
- **[CONSTANTS.md](../CONSTANTS.md)** - Mathematical and system constants
- **[TUNING.md](../TUNING.md)** - Performance tuning parameters and optimization settings

## Architecture & Design

### System Architecture
- **[ARCHITECTURE.md](../ARCHITECTURE.md)** - Overall system architecture, design principles, and high-level organization
- **[SOFTWARE_MODULES.md](architecture/SOFTWARE_MODULES.md)** - Detailed reference for all software modules, their purposes, and I/O variables
- **[STATE_MANAGEMENT_REFACTORING.md](STATE_MANAGEMENT_REFACTORING.md)** - Phase 3.4 state management implementation details

### Architecture Guidelines
- **[docs/architecture/README.md](architecture/README.md)** - Architecture documentation overview
- **[docs/architecture/01_PRINCIPLES.md](architecture/01_PRINCIPLES.md)** - Architectural principles and patterns
- **[docs/architecture/02_INFORMATION_FLOW.md](architecture/02_INFORMATION_FLOW.md)** - Data flow and communication patterns
- **[docs/architecture/03_MODULE_DIRECTORY.md](architecture/03_MODULE_DIRECTORY.md)** - Module organization and dependencies
- **[docs/architecture/04_API_CONTRACTS.md](architecture/04_API_CONTRACTS.md)** - API interfaces and contracts
- **[docs/architecture/05_CODING_GUIDELINES.md](architecture/05_CODING_GUIDELINES.md)** - Coding standards and best practices
- **[docs/architecture/07_PERFORMANCE_TARGETS.md](architecture/07_PERFORMANCE_TARGETS.md)** - Performance requirements and benchmarks
- **[docs/architecture/08_VALIDATION_CHECKLIST.md](architecture/08_VALIDATION_CHECKLIST.md)** - Validation procedures and checklists

### Architecture Decisions & Refactors
- **[docs/architecture/baseline_storage_refactoring.md](architecture/baseline_storage_refactoring.md)** - Baseline data storage improvements
- **[docs/architecture/perspective_management_refactor.md](architecture/perspective_management_refactor.md)** - Perspective management enhancements
- **[docs/architecture/spinbox_refactor_2025_12.md](architecture/spinbox_refactor_2025_12.md)** - Spinbox component refactoring
- **[docs/architecture/spinbox_audit_phase1.md](architecture/spinbox_audit_phase1.md)** - Spinbox audit and analysis
- **[docs/architecture/entry_point_consolidation_plan.md](architecture/entry_point_consolidation_plan.md)** - Entry point consolidation strategy

## User Guides

### Interactive Editor
- **[interactive_editor_user_guide.md](interactive_editor_user_guide.md)** - Complete user guide for the interactive scenario editor
- **[INTERACTIVE_EDITOR_CHANGELOG.md](INTERACTIVE_EDITOR_CHANGELOG.md)** - Version history and change log
- **[interactive_edit_roadmap.md](interactive_edit_roadmap.md)** - Development roadmap and future features
- **[interactive_edit_ph2_requirements.md](interactive_edit_ph2_requirements.md)** - Phase 2 requirements and specifications

### Configuration & Scenarios
- **[SCENARIO_CONFIGURATION_GUIDE.md](SCENARIO_CONFIGURATION_GUIDE.md)** - How to configure and customize scenarios
- **[scenario_generator_requirements.md](scenario_generator_requirements.md)** - Scenario generator specifications
- **[scenario_generator_build.md](scenario_generator_build.md)** - Scenario generator implementation details

## Development & Testing

### Testing Framework
- **[INTERACTIVE_EDITOR_TESTING.md](INTERACTIVE_EDITOR_TESTING.md)** - Testing methodology, procedures, and MVT standard
- **[Validation.md](Validation.md)** - Validation procedures and quality assurance
- **[../verification/README.md](../verification/README.md)** - Automated verification scripts and baseline state logs

### Debugging & Development
- **[DEBUG.md](DEBUG.md)** - Debugging guide, logging configuration, and troubleshooting
- **[docs/architecture/OBSERVABILITY_IMPLEMENTATION_COMPLETE.md](architecture/OBSERVABILITY_IMPLEMENTATION_COMPLETE.md)** - Observability system implementation
- **[State_Viewer.md](State_Viewer.md)** - State viewer documentation
- **[example_state_log.txt](example_state_log.txt)** - Example state logging output

### Development Tools
- **[editor_config_example.json](editor_config_example.json)** - Example editor configuration
- **[installation_4_interactive_editor.md](installation_4_interactive_editor.md)** - Installation guide for interactive editor

## Research & Theory

### GRP Theory & Research
- **[REV4_IMPLEMENTATION_SUMMARY.md](REV4_IMPLEMENTATION_SUMMARY.md)** - GRP Revision 4 implementation summary
- **[fidelity_asymmetry_research_questions.md](fidelity_asymmetry_research_questions.md)** - Research questions on fidelity asymmetry
- **[future_investigations.md](future_investigations.md)** - Future research directions and investigations

### Gamma Self Research
- **[gamma_self_defense.md](gamma_self_defense.md)** - Gamma self defense mechanisms
- **[gamma_self_love_space_defined.md](gamma_self_love_space_defined.md)** - Definition of gamma self love space
- **[Gamma_Self_Character_Map.png](Gamma_Self_Character_Map.png)** - Visual representation of gamma self characteristics

### Communication & Protocols
- **[baseline_communication_protocol.md](baseline_communication_protocol.md)** - Communication protocol specifications
- **[rosetta_stone_translator.csv](rosetta_stone_translator.csv)** - Translation mappings and equivalences

## Configuration & Setup

### Build & Deployment
- **[Dockerfile](../Dockerfile)** - Container configuration
- **[requirements.txt](../requirements.txt)** - Python dependencies
- **[pytest.ini](../pytest.ini)** - Testing configuration

### Project Management
- **[FUNDING.md](../FUNDING.md)** - Funding and sponsorship information
- **[LICENSE.md](../LICENSE.md)** - Project license
- **[file_list.md](../file_list.md)** - Complete file inventory

## Legacy & Archive

### Archived Documentation
Located in **[docs/archive/](archive/)**:
- **[Design_Rationale.md](archive/Design_Rationale.md)** - Original design rationale
- **[GRP_rev3.1.md](archive/GRP_rev3.1.md)** - GRP Revision 3.1 specification
- **[GRP_rev3.3.md](archive/GRP_rev3.3.md)** - GRP Revision 3.3 specification
- **[GRP_rev3.md](archive/GRP_rev3.md)** - GRP Revision 3 specification
- **[GRP_rev4.md](archive/GRP_rev4.md)** - GRP Revision 4 specification
- **[ID_BASED_REFACTOR_COMPLETE.md](archive/ID_BASED_REFACTOR_COMPLETE.md)** - ID-based refactoring completion
- **[Slope_Based_Generation_Implementation.md](archive/Slope_Based_Generation_Implementation.md)** - Slope-based generation implementation
- **[UREP.md](archive/UREP.md)** - UREP framework documentation
- **[UREP_2025_Simplification_Proposal.md](archive/UREP_2025_Simplification_Proposal.md)** - UREP simplification proposal
- **[weights_defense.md](archive/weights_defense.md)** - Weights defense mechanisms

### Superseded Documentation
Located in **[docs/archive/superseded/](archive/superseded/)**:
- Legacy documentation that has been replaced by current versions

## Navigation Guide

### Quick Start
1. **[README.md](../README.md)** - Start here for project overview
2. **[interactive_editor_user_guide.md](interactive_editor_user_guide.md)** - Learn to use the interactive editor
3. **[ARCHITECTURE.md](../ARCHITECTURE.md)** - Understand system design
4. **[SOFTWARE_MODULES.md](docs/architecture/SOFTWARE_MODULES.md)** - Find specific module details

### Development Workflow
1. **[interactive_edit_roadmap.md](interactive_edit_roadmap.md)** - See development priorities
2. **[INTERACTIVE_EDITOR_TESTING.md](INTERACTIVE_EDITOR_TESTING.md)** - Follow testing procedures
3. **[DEBUG.md](DEBUG.md)** - Troubleshoot issues
4. **[docs/architecture/05_CODING_GUIDELINES.md](architecture/05_CODING_GUIDELINES.md)** - Follow coding standards

### Research & Theory
1. **[grp_principles.md](grp_principles.md)** - Understand core concepts
2. **[WHY_THIS_MATTERS.md](../WHY_THIS_MATTERS.md)** - See practical applications
3. **[REV4_IMPLEMENTATION_SUMMARY.md](REV4_IMPLEMENTATION_SUMMARY.md)** - Review latest GRP implementation

### Architecture Deep Dive
1. **[ARCHITECTURE.md](../ARCHITECTURE.md)** - High-level architecture
2. **[docs/architecture/README.md](architecture/README.md)** - Architecture documentation overview
3. **[SOFTWARE_MODULES.md](docs/architecture/SOFTWARE_MODULES.md)** - Module-by-module breakdown
4. **[docs/architecture/02_INFORMATION_FLOW.md](architecture/02_INFORMATION_FLOW.md)** - Data flow patterns

---

**Last Updated:** December 17, 2025
**Total Documents:** 60+ files across 4 main directories
**Active Documents:** 45+ current specifications and guides
**Archived Documents:** 15+ legacy specifications (preserved for historical reference)</content>
<parameter name="filePath">c:\Users\jeffg\Documents\GitHub\WhenMathPrays\docs\CONTENTS.md