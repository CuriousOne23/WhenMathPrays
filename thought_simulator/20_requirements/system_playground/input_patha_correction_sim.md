...
The purpose is to show that **Path A correction is robust** even under degraded input
conditions, and that it can still produce a coherent TP(N+1) + semantic_core without
hallucination, grammar repair, or semantic inference.

This simulation was executed **mechanically and a‑priori** by **Microsoft Copilot**,
acting as the **simulation engine**, following only the behaviors defined in the
applicable **20_requirements** documents. These include:
...
Microsoft Copilot executed each primitive step strictly under these constraints,
ensuring that the simulation reflects **only** the defined behavior of Path A,
without hindsight, correction, or semantic inference.
...
- Anomaly‑handling table (which primitive caught what, and what action was taken)
- TPU commit block: `commit_id`, `semantic_core` snapshot, `commit_status`,
  `missing_mass`
...
Produces the **Intake Envelope** (normalized, schema‑validated surface form).
Does **not** correct spelling or grammar.
Sets `intake_envelope_status` to `valid_clean`, `valid_degraded`, or `valid_minimal`.

### **IIInB — Input‑Side Inference Basin**
Detects local structural cues (roles, adjacency, anomalies).
Assigns `MI_class` tag from the messy‑input taxonomy (MI_INCOMP, MI_VAGUE,
MI_AFFECT, MI_NOISE, MI_CONTRA).
...
Evaluates structural and lexical plausibility of each candidate.
Assigns `structural_score` and `lexical_score`.
Emits `warnings` where applicable.

### **ISc — Inference Scorer**
Computes `ΔH%` (normalized per‑lane hypothesis mass delta) and `confidence`.
Applies `threshold`; flags warnings when `confidence < threshold`.

### **TPU — Thought Processing Unit**
Commits `TP(N+1)` and `semantic_core` exactly as produced by the pipeline.
Records `commit_id`, `commit_status`, and `missing_mass`.
...
- **MI_INCOMP** (4 cases: 1, 7, 8, 10) — Missing slots are the most common
  structural failure mode. All cases committed with missing_mass accounting.
- **MI_VAGUE** (3 cases: 2, 4, 5) — Ambiguity is structurally preserved, never
  collapsed. ΔH% = 0.00 in all three — correct, as no new mass is committed
  under ambiguity.
- **MI_NOISE** (5 cases: 3, 9, 11, 12, 13, 14) — Surface-level errors are
  absorbed by the structural
