...
It ensures clean, stable, versioned, and observable interaction between the core TS engine and external tools, scripts, observer layers, and human users â€” while preserving determinism, architectural isolation, and reproducibility.

## 2. Core Interface Principles

* All interfaces are **read-only or command-based** from the core TS engineâ€™s perspective.
...
* All I/O must support bitwise reproducibility (same seed + config + snapshot â†’ identical outputs).

## 3. Configuration Interface

**IO-CFG-01: Configuration Format**  
- Primary format: YAML with JSON schema validation.
- Single entry point: a project configuration YAML file (or `--config <path>` CLI flag).
...
**IO-DAT-06: Artifact Directory and Schema Documentation**  
- Verification artifacts must be written under dedicated artifact directories (for example, `artifacts/`) rather than module source roots.
- Module-level software descriptions must document the active IO schema version, compatibility policy, and expected artifact paths for their outputs.
...
- All interfaces must be covered by determinism and reproducibility tests (see 20.60_testing_and_validation.md).
...
**Version**: 0.3  
**Changes from 0.2**:
- Added artifact directory discipline for verification outputs.
- Added requirement to document module IO schema compatibility and artifact path expectations.

**Changes from 0.1**:
- Incorporated all five of Copilotâ€™s refinements (No Hidden Global State, Round-Trippability, Machine-Readable CLI Output, Live Metrics Stream Format, Interface Determinism Tests).
