...
TPU sits at the end of the Correction Group. After TPU, Path A proceeds
through IMR and then the OB‑set structural interpretation group:

    InB → IIInB → IE → ISc → CEx → CE → TPU → IMR
        → SOB → SROB → CnOB → SmOB → SSG

OB‑set constructs the stabilized structural geometry that enables routing and prepares the TP for semantic interpretation by IdOB.
...
### **Clarifying‑Field Commit Requirements (New)**

### **HLR‑20.46‑017**  
TPU **shall** validate clarifying‑field metadata contained in `tp_update_request`, including clarifying fields, clarifying subfields, hierarchical clarifying sub‑subfields, clarifying importance, and clarifying topology.

### **HLR‑20.46‑018**  
TPU **shall** enforce writer authority for clarifying‑field metadata exactly as defined in 20.105, permitting updates only from authorized primitives (CIL, COB, and Path‑A inference sources).

### **HLR‑20.46‑019**  
TPU **shall** commit clarifying‑field metadata atomically, ensuring that clarifying fields, subfields, and importance scores are updated as a single deterministic unit.

### **HLR‑20.46‑020**  
TPU **shall** preserve clarifying‑field provenance when committing updates, including producer identity, last‑update identity, and commit lineage.

### **HLR‑20.46‑021**  
TPU **shall** enforce canonical ordering for clarifying‑field metadata, including deterministic ordering of clarifying fields, subfields, hierarchical levels, and importance scores.

### **HLR‑20.46‑022**  
TPU **shall** maintain clarifying‑field continuity across TP(N) → TP(N+1), ensuring that clarifying fields are not lost, reordered, or mutated outside authorized updates.

### **HLR‑20.46‑023**  
TPU **shall** include clarifying‑field metadata in `tpu_audit_record{}` whenever clarifying fields or clarifying importance are updated, pruned, compressed, or dropped due to bounded limits.

### **HLR‑20.46‑024**  
TPU **shall** apply clarifying‑field updates with the same 1‑TP‑cycle lag semantics as all other TP metadata, making clarifying‑field changes visible only in TP(N+1).

### **HLR‑20.46‑025**  
TPU **shall** reject any clarifying‑field updates that violate bounded limits (maximum 10 fields, 100 subfields, 4 hierarchical levels), producing deterministic fallback behavior and audit.

### **HLR‑20.46‑026**  
TPU **shall** guarantee deterministic replay of clarifying‑field metadata such that identical sequences of update requests produce identical clarifying‑field state in TP(N+1).
...
- audit logging  
- 1‑TP‑cycle lag semantics
- interpretation of ΔH% strictly according to the unified structural‑entropy delta defined in HLR‑20.030‑015 (20.30 §2.6)
...
TPU SHALL write updates only to:
- **TP.semantic** (semantic content from ISc)
- **TP.process** (ordering metrics, stability metadata, identity‑layer selection)
- **TP.metadata** (audit, provenance, safe‑boundary markers)
...
Fallback SHALL be deterministic and SHALL NOT mutate TP.semantic, TP.process, or TP.metadata.
