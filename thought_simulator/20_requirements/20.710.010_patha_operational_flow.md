...
- the **Four‑Field Token Model (FFTM)**  
...
InB → CEx → CE → {IMR} → SOB → SROB → CnOB → SmOB → ISc → SSG → STPX → RBU → DCB → TR → CTP → {IMR} → ISc → RTU → {IMR} → RB → IdOB → RBU → {IMR} → DCB → TR → OuBA
...
InB → IIInB → IE → CEx → CE → ISc → TPU → {IMR} → SOB → SROB → CnOB → SmOB → ISc → SSG → STPX → RBU → {IMR} → DCB → TR → CTP → {IMR} → ISc → RTU → {IMR} → RB → IdOB → RBU → {IMR} → DCB → TR → CTP → {IMR} → ISc → RTU → {IMR} → RB → 

IdOB → RBU → {IMR} → DCB → TR → OuBA
...
- IdOB — performs identity‑conditioned meaning refinement after routing
...
# **14. Conversation‑Layer Overview (OuBA → SSRGn → COB → CIL → CST → CEx)**  
*(Light, simulation‑aligned overview)*

The conversation layer processes the committed Path A snapshot and prepares context for the next turn.  
It is intentionally summarized here; deeper behavioral details belong in the specific requirement documents (20.32, 20.33, 20.32.010, 20.54) or in future simulation‑driven papers.

### **Operational Flow (Horizontal)**  
```
OuBA → SSRGn → COB → CIL → CEx
             → CST →
*Note: CST is parallel to COB, consumes SSRGn and inputs into COB to keep COB stable
```

### **Purpose (High‑Level)**  
- **OuBA** finalizes Path A meaning.  
- **SSRGn** produces the unified A→B boundary artifact.  
- **COB** maintains conversation‑level identity layers and referent continuity.  
- **CIL** provides identity‑conditioned context for the next turn.  
- **CST** monitors & mititgates stability and drift of COB.  
- **CEx** uses this context to prepare correction candidates for the next turn.

### **Simulation Note**  
The conversation layer is **sufficiently specified** for Path A simulations, but several behaviors (e.g., identity‑layer pruning, stability thresholds, conversation segmentation, optional metadata retention) will require deeper definition.  
These details should be addressed in **future requirement documents or simulation‑focused papers**, not expanded here, to keep this operational flow document concise and aligned.
