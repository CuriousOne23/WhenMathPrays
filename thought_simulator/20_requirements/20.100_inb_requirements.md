...
Its purpose is to ingest raw external signals and convert them into **bounded, canonical, schema‑validated input** without performing any semantic interpretation or truth arbitration.
...
- **Deterministic ingestion** of external signals into TP input fields  
...
- **Isolation of transport/session metadata** from semantic payloads  
- **Deterministic provenance recording** for audit and replay  
- **Non‑inferential normalization** of noisy or malformed input  
- **Strict separation** between raw input handling and semantic repair (delegated to IIInB/UPI)

InB is the **entry point** of the entire inference pipeline.  
It guarantees that all downstream components—IIInB, CIL, CEx, CE, ISc, TPU, and TP(N+1)—receive input that is **clean, bounded, canonical, and free of semantic assumptions**.
...
19. HLR-20.100-019: InB SHALL NOT read or mutate IdOB, RB, TB, IB, GB, CIL, COB, COP, or MTP internal state and SHALL operate only on intake-bound fields and approved interfaces.
