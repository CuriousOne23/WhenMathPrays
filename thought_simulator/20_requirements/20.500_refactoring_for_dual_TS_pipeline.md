...
**Phase 0 also resolves (GATE — program decision, not runtime invariant):** Treatment of [20.12](20.12_ts_invariants.md) during this refactor — **annex/glossary cross-refs** vs **explicit no-touch** (OQ-003, §4.2). **Decision (Grok + Copilot aligned; CuriousOne23 confirm on P0 sign):** **no-touch** for 20.12 body text; resolve naming collisions via §5 registry and Stage 4 per-doc edits only; annex only if a specific cross-ref gap blocks 20.206 (requires **GATE**).
...
| OQ-003 | 20.12 annex vs no-touch during refactor? | All three | `resolved` | **No-touch** for 20.12 body; §5 registry + Stage 4 per-doc for naming; annex only via GATE if 20.206 blocked (Grok + Copilot; CuriousOne23 confirm on P0 sign) |
