...
**How to use this document:** Treat §2–§3 as the execution contract. Update status columns in §7 as work progresses. **Resume here → §7.5.** Phase 0 signed; Stage 1a complete; **Stage 2 drafts complete 2026-06-06** (all §7.1 rows `draft`, unapproved). **Current GATE: Stage 4** three-way review of Stage 2 stack — or parallel **§6 CI manifest** (1.5b) / **Stage 3** placeholders (20.10, 20.30, 20.36).
...
| Stage 3 placeholders | Stage 4 priority (optional parallel) | 2026-06-06 | 20.10 / 20.30 / 20.36 only |
| 20.120 schema update | 20.115 Stage 4 approval | 2026-06-06 | Thin v0.1; needs `commit_id` + 20.115 D8/D11 |

### 7.5 Resume Here (2026-06-06 pause point)
...
**Recommended next actions (pick one track):**

| Track | Action | Owner |
|-------|--------|-------|
| **A — Review** | Three-way pass on Stage 2 stack; start with 20.206 → 20.115 → 20.205 | All three |
| **B — CI** | Complete §6 manifest checkboxes; approve first 1.5b allowlist (glossary registries first) | CuriousOne23 + Copilot |
| **C — Schema** | After 20.115 review: harden **20.120** with `commit_id`, reconcile HLR-20.120-006 vs `mtp_snapshot_id` | Stage 4 |
| **D — Stage 3** | Additive placeholders in 20.10 / 20.30 only (no B normative insertions) | CuriousOne23 + Copilot |

**Cross-draft review flags (not in §7.3 — resolve during Stage 4):**
- **D17** — TrigRB vs SRP stage order: 20.57 (`trig_rb` first) vs 20.36 §2.2 (`srp_lookup` first)
- **D8** — `commit_id` hash wire format (20.115, 20.120, 20.39)
- **D2** — `semantic_snapshot_ref` hash vs embedded copy (20.206, 20.207, 20.39)

**Not started / thin (do not confuse with Stage 2 drafts):** 20.120 (attached — 8 HLRs, pre-`commit_id`), 20.10, 20.30, 20.36 updates, 20.190 glossary, 20.200 traceability matrix.
