...
**XP: Execution Plan:** A regenerable B‑side plan that specifies how the committed meaning will be realized. XP provides the Realizer with deterministic instructions (behavior plan, voice/identity graph, translation routine, routing_epoch bindings) needed to produce surface output. XP never carries meaning, never influences Path‑A, and is regenerated from the committed semantic snapshot each cycle.
