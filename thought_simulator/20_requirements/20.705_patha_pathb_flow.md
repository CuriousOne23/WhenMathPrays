...
- **CoHI (Conversation History Integrator):**  
  Reads:
  - the **A→B TP** (including `ssr.truth_tags`, `tp.safety_flags`), and  
  - **COB/CIL** structures (structural_history, referential_history, conversation_objects, topic_threads, continuity_markers).  

  CoHI projects these into **`tp.continuity_fields`**—a per‑turn continuity substrate attached to the SSR.  
...
- **LI (Local Inference Primitive) + MLI‑prc:**  
  ...
  LI:
  - reads representation‑layer fields, `ssr.truth_tags`, `ssr.safety_flags`, and `tp.continuity_fields`,  
  - invokes `MLI‑prc`,  
