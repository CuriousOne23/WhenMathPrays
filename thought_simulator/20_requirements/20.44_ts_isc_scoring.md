...
ISc SHALL NOT write directly to TP.semantic, TP.process, or TP.metadata.  ISc outputs SHALL be written only to the isc{} block and SHALL reach TP exclusively through Merge → TPU → TP.process.isc.
...
### **HLR‑20.44‑066 — Discourse‑Context Cue Consumption**  
ISc **SHALL** consume normalized discourse‑context structural cues emitted by STPX — including recent‑entity continuity, temporal‑shift markers, contrastive structural markers, and causal structural markers — and **SHALL** incorporate these cues into the scoring feature set in a strictly structural, non‑semantic manner.

### **HLR‑20.44‑067 — Structural‑Only Interpretation of Discourse Cues**  
ISc **SHALL NOT** perform semantic interpretation, stance inference, identity resolution, or meaning reconstruction when consuming discourse‑context cues, and **SHALL** treat all such cues strictly as structural scoring features.

### **HLR‑20.44‑068 — Deterministic Discourse‑Cue Scoring**  
ISc **SHALL** apply deterministic scoring to discourse‑context cues, ensuring that identical cue_envelope inputs produce identical scoring contributions across all executions, consistent with replay invariants (20.12).

### **HLR‑20.44‑069 — Integration of Discourse Cues into Rationale Metadata**  
ISc **SHALL** include discourse‑context cues in rationale metadata when they contribute to candidate ranking, ensuring transparent, auditable scoring behavior consistent with HLR‑20.44‑005.

---

## **Next‑Turn Context Handling (TP.next_context_fields Cross‑Reference)**

### **HLR‑20.44‑070**  
ISc SHALL read next‑turn clarifying/context fields exclusively from **CE.context_fields{}**, as extracted by CEx from `TP.next_context{}` defined in **20.105_tp_requirements.md**.

### **HLR‑20.44‑071**  
ISc SHALL treat next‑turn context fields as **pre‑semantic structural metadata** and SHALL NOT modify, reinterpret, or repair their values.

### **HLR‑20.44‑072**  
ISc SHALL include next‑turn context fields in its scoring‑feature preparation exactly as provided in CE, without renaming, omission, or structural modification.

### **HLR‑20.44‑073**  
ISc SHALL use next‑turn context fields (topic, stance, intent, register, politeness, epistemic_shading, continuity, direction, coherence, shift_required, importance) as **structural scoring features**, not semantic features.

### **HLR‑20.44‑074**  
ISc SHALL NOT derive next‑turn context fields from clarifying fields, identity‑layer metadata, referent maps, or structural cues; next‑turn context SHALL originate exclusively from CE.

### **HLR‑20.44‑075**  
ISc SHALL guarantee deterministic replay of next‑turn context handling; identical CE input SHALL produce identical scoring‑feature preparation.

### **HLR‑20.44‑076**  
ISc SHALL NOT perform semantic inference, stance inference, identity resolution, or meaning reconstruction when consuming next‑turn context fields.

### **HLR‑20.44‑077**  
ISc SHALL preserve next‑turn context continuity across turns by passing next‑turn context fields to TPU unchanged through the isc{} block.

### **HLR‑20.44‑078**  
ISc SHALL NOT compute context coherence or context shift; it SHALL only reflect `coherence` and `shift_required` values provided by CEx.

### **HLR‑20.44‑079**  
ISc SHALL include next‑turn context fields in TPU update requests (`TPU.isc_update`) exactly as defined in **20.105**, without modification.

### **HLR‑20.44‑080**  
ISc SHALL treat missing or partially populated next‑turn context fields as deterministic empty‑context cases and SHALL populate TPU update requests with canonical empty values.

### **HLR‑20.44‑081**  
ISc SHALL NOT interact with truth/done fields, routing fields, structural geometry, ΔH%, lineage, or any Path‑B semantics when processing next‑turn context.

### **HLR‑20.44‑082**  
ISc SHALL record any next‑turn context drops, truncations, or normalization adjustments in `isc_output.rationale_metadata`, following the same deterministic rules used for other structural scoring features.
...
ISc outputs SHALL become visible in TP.process.isc(N+1) after TPU commit.
...
ISc SHALL write only to the isc{} block, which TPU SHALL commit into TP.process.isc.
ISc SHALL NOT modify TP.semantic, TP.process, TP.metadata, TP.context, or TP.intake.
