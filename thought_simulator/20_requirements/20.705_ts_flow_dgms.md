...
HC‑Flow consumes the historical TP sequence written by **OuBB** (Output Basin — Path B commit) and transforms it through **MTP**, **HCF**, and **COB** into structured conversation‑level objects. These objects are interpreted by **CIL**, which produces long‑horizon conversational inference used by Path B during realization.
...
- **HCF — Historical Conversation Flow**  
  Filters and normalizes historical TP content for downstream processing.

- **COB — Conversation Object Basin**  
  Constructs structured conversation‑level objects from filtered historical TP data.

- **CIL — Conversation Integration Layer**  
  Produces long‑horizon conversational inference from COB outputs.
...
- **CIL — Conversation Integration Layer**  
  Produces long‑horizon conversational inference.

- **CoHI — Conversation History Integrator**  
  Integrates conversation‑level continuity fields into Path B.
