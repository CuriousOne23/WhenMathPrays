...
This document defines the **precise, deterministic program flow** of the Thought Simulator (TS) â€” the ordered sequence of operations executed on every tick.
...
* All phases are designed to be **parallel-friendly** (see [20.40_performance_requirements.md](20.40_performance_requirements.md)).
* The flow must preserve the external Observer boundary â€” no observer logic runs inside the core tick.
* Every state-changing action increments the **State Counter** and is logged (see 20.50_observability_requirements.md).
...
* Any phase detecting a critical violation immediately triggers **Fail-safe Termination** (freeze state â†’ flush logs â†’ atomic snapshot â†’ close).
...
* **Visualization** (18): Consumes exported data only â€” never runs inside the tick cycle.
...
* A researcher can trace any ThoughtPointâ€™s behavior by examining logs and snapshots and reconstruct the exact sequence of phases it experienced.
...
- Added explicit â€œNo cross-phase mutationâ€ rule.
