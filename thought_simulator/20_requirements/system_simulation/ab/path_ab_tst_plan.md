...
The AB1–AB8 suite is not specific to any one architecture. These eight tests represent the minimal, complete, and orthogonal set of failure modes, driven by safety concerns, that any cognitive or reasoning system can exhibit, regardless of implementation details. The eight proposed tests attempt to answer the question, "What happens when something goes wrong?" Regardless of the reason. Any system that processes meaning, maintains internal state, and produces structured output can fail in only eight distinct ways: (1) failure on the clean path, (2) failure at boundaries, (3) failure under degraded input, (4) failure under structural faults, (5) failure of isolation when multiple thought streams are active, (6) failure of semantic integrity within its internal representations, (7) failure to correctly handle contradictions within the content itself, and (8) failure to maintain stability over extended sequences (drift). Though the tests are safety‑driven, the safety emphasis naturally exposes the architectural weaknesses and assumptions of any design. Because TS is a new architecture, these tests also serve as a practical way to understand its structure and invariants.
