...
**Version:** 0.2  
**Date:** 2026-06-06  
**Status:** Draft — dual-pipeline replay taxonomy (PoC gating)
...
16. **HLR-20.036-016:** PoC acceptance SHALL require pass on all five replay test classes before Execution Manifold contracts are considered proven per HLR-20.010-114.

17. **HLR-20.036-017:** Each test class SHALL define: `fixture_id`, `preconditions`, `injection_steps`, `required_assertions`, and `strip_scope` (which envelopes to remove for replay diff).

18. **HLR-20.036-018:** Replay diff SHALL use canonical serialization ordering per 20.95 for all compared fields.
...
strip_scope: [exec_plan, exec_trace]
injections:
  - messy_input_class: [MI_CONTRA, MI_VAGUE]
  - overflow_type: LANE_TRUNCATION
  - imr_trigger_type: B
  - gb_mode_path: NORMAL -> DAMPENED
assertions:
