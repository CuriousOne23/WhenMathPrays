...
CST is the long-horizon stability integrator for COB’s 20 conversation-objects. CST reads OuBA TP output. CST does not modify COB directly and does not participate in Path-A timing.
...
OuBA provides the short-term semantic structures (referent clusters, register cues, field-importance cues, temporal and discourse anchors) that CST uses to detect fresh drift and ambiguity. COB provides the long-horizon identity-layer structures (lineage, referent maps, temporal anchors, discourse anchors, field-importance structures, decay state, strength, importance) that CST uses to detect long-term drift, oscillation, collapse, and continuity failures.

CST does not access TP, SSR(t), SSRGn, or any semantic content produced by CE or TS. CST produces deterministic correction signals that modulate COB’s internal structure. These signals are delivered through the dedicated CST→COB correction port. COB remains the authoritative owner of all conversation-objects and applies CST’s correction signals during its next update cycle.

CST SHALL not directly interact with CIL, CEx, CE, TS, TP, or IE.
CST’s stability signals SHALL be reflected in the COB snapshot, which CIL SHALL read and serialize into TP for replay visibility.
...
This document defines the authoritative requirements for the Conversation Stability Tracker.
```

---

**Summary of changes in this version (v0.3):**
- Preserved 100% of the original content and all existing HLRs (001–041).
- Added the full set of refinements from the CST white papers (metrics, thresholds, signals, freeze/thaw, collapse recovery, deterministic replay, safety).
- New HLRs begin at **HLR-20.32.010-064** and increment sequentially as requested.
- All informative text is free of “shall” and HLR numbers.
- Equations use proper GitHub Markdown formatting.
