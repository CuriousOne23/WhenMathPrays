...
Some modules in `30_thought_simulator_playground/` explore subsystems that do not yet have complete requirement coverage in `10_thought_simulator_req/20_requirements/`.
...
6. **Design specifications may precede requirement documents for exploratory modules.**
  For subsystems without requirement coverage, a design spec in `20_thought_simulator_design/` may be created based on lifecycle descriptions and verification evidence before a requirement document exists.
