...
> COB behavior stays here. Envelope root `TP.cob`: `20.116.010`. `pack_ids[]` is not a register.
...
# ✅ **NEW SECTION — 16. Path‑A TP Support Requirements (Normative)**  
*(Paste this directly into 20.32_cob_requirements.md)*

### **HLR‑20.32‑130**  
COB SHALL expose `TP.cob.identity_lineage` derived deterministically from identity layers and their `lineage` structures.

### **HLR‑20.32‑131**  
COB SHALL expose `TP.cob.continuity_lineage` derived from lineage‑continuity markers, context‑shift markers, and next‑turn context integration.

### **HLR‑20.32‑132**  
COB SHALL expose `TP.cob.topology` describing the identity‑layer and referent‑map graph structure, including nodes, edges, and layer relationships, in a compact deterministic format.

### **HLR‑20.32‑133**  
COB SHALL maintain `TP.cob.metrics` containing bounded‑semantic scalar fields required by Path‑A:  
- `ambiguity_score`  
- `collapse_risk`  
- `drift_score`  
- `stability_score`  
- `lineage_confidence`  
computed deterministically from identity‑layer state, CST signals, recency/frequency/density metrics, importance continuity, and lineage continuity.

### **HLR‑20.32‑134**  
COB SHALL expose `TP.cob.metrics` in the stabilized snapshot for consumption by CIL and subsequent mapping into `TP.cil.metrics`.

### **HLR‑20.32‑135**  
COB SHALL expose `TP.cob.register_continuity` derived from identity‑layer register fields and continuity markers for consumption by CIL.

### **HLR‑20.32‑136**  
COB SHALL expose `TP.cob.importance_continuity` derived from long‑horizon importance integration, recency/frequency/density metrics, lineage continuity, next‑turn context, and CST correction signals.

### **HLR‑20.32‑137**  
COB SHALL expose all fields listed in HLR‑20.32‑130 through HLR‑20.32‑136 in the stabilized identity‑layer snapshot consumed by CIL.

# **17. Path‑A IdOB Conversation Continuity (Normative)**

Informative. COB remains the long‑horizon identity substrate. It does not run the IdOB crossing and does not load dictionaries. It records a bounded freeze of the OuBA meaning packet so the next turn can recover names, pack keys, leftover residue, and a slim hop snapshot. `register` on an identity layer stays social/discourse register. Pack identity is a separate key.

### **HLR‑20.32‑138**
COB SHALL ingest meaning exclusively through OuBA packets (HLR‑20.32‑016) and SHALL treat the following fields as the authorized IdOB-continuity subset when present on the OuBA packet:
- `utterance` (carrier string, if present)
- `conversation_id`
- `pack_ids[]` (dictionary / subculture pack keys loaded or requested for this turn)
- slim `TP.idob` snapshot: `structural_key`, `selected_group_id`, `meaning_delta_h`, `residue_code`
- optional referent hints: `surface_forms[]` offered for name/referent continuity

### **HLR‑20.32‑139**
COB SHALL persist the authorized IdOB-continuity subset in the identity-layer / referent structures of the selected conversation and SHALL expose those fields in the stabilized snapshot consumed by CIL.

### **HLR‑20.32‑140**
COB SHALL treat `pack_ids[]` as pack keys only. COB SHALL NOT derive `pack_ids[]` from the identity-layer `register` field, from CEx-IE `register_hint`, or from six-axis meaning values.

### **HLR‑20.32‑141**
COB SHALL treat identity-layer `register` as social/discourse register continuity. Register SHALL remain distinct from `pack_ids[]`.

### **HLR‑20.32‑142**
COB SHALL merge optional referent `surface_forms[]` from the OuBA packet into the referent map of the selected layer using existing referent-map rules (HLR‑20.32‑015). COB SHALL NOT invent names or facts that are absent from the OuBA packet and the existing referent map.

### **HLR‑20.32‑143**
COB SHALL NOT interpret six-axis meaning vectors, CIE stance, or `meaning_delta_h` as identity-layer topology. Those fields MAY be stored as continuity metadata; they SHALL NOT trigger Create, Split, Merge, Freeze, or Thaw.

### **HLR‑20.32‑144**
COB SHALL NOT load structure or meaning dictionaries. Pack loading is an IdOB / structure-assignment concern. COB only records which `pack_ids[]` the OuBA packet reported.

### **HLR‑20.32‑145**
COB SHALL guarantee deterministic replay of the IdOB-continuity subset across freeze/thaw cycles.

### **HLR‑20.32‑146**
When an OuBA packet omits any field of the IdOB-continuity subset, COB SHALL treat the omitted field as a deterministic empty/absent case and SHALL NOT invent a substitute from IE structural cues.
