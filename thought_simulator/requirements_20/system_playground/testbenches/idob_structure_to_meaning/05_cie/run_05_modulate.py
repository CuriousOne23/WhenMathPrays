"""Slide 05 / Stop 5 — Identity moves M. Structure does not change.

Formula: M' = clip(M + alpha * I)
Does not rewrite structural_key. Does not invent map candidates.
run() returns a record; driver ignores the return.
"""
...
def run(group_id=1001, cie_id=None, clip_to_unit=True):
    ...
    print("=" * 64)
    print("LESSON 05 / STOP 5 — CIE MODULATION")
    ...
    record = {
        "group_id": group_id,
        "M": M,
        "clip_to_unit": bool(clip_to_unit),
        "rows": [],
    }
    if M is None:
