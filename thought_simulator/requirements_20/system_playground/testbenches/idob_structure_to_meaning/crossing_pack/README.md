...
- Writes:
    - nothing
- Why no write:
    - placement is a return value only; gold stays the fixture
- Look for:
    - returned dict + hole list on the terminal if you print

#### probe_log.py
- Reads:
    - whatever place()
        - reads (Seed YAML via place.py)
- Writes:
    - logs/probe.log (default) or log_path you pass
- Why write:
    - Stop 1 diary so tally() can count gold vs unseen
    - Does not write:
        - Seed YAML
        - live cards
- Look for:
    - one JSON line per probe(); tally dict on terminal

#### suggest_tags.py
- Reads:
    - place() / Seed YAML
        - allow-lists in the module (must match Seed about/family ids
        - if the code does not open those YAML files
            - say “allow-lists copied in suggest_tags.py, not read live”)
- Writes:
    - logs/suggest.log (default) or log_path
- Why write:
    - Stop 2 candidate diary
        - local .log not committed
    - Does not write:
        -  placements.yaml
        -  cards
        -  Door Table
...
#### test_*.py
- Reads:
    - the module under test + Seed YAML through place/suggest
- Writes:
    - temp logs only (pytest tmp_path)
- Does not write:
    - logs/probe.log
    - logs/suggest.log
    - seed/*
