...
- Handoff shapes for [40.170](../40.170_split_merge_prototypes/software_description.md) split/merge and [40.501](../40.190_rb_prototypes/software_description.md) routing

**Agreement Statement (W3 Phase A)**: Aligned — CP review 2026-06-08 confirms W3 targeted redo scope (intake-bound repair fields, `commit_id` boundary, lane-local A-chain semantics, 40.170/40.501 handoffs) with no structural defects. Legacy macro API and `tp_lifecycle_io_schema_v1` remain valid; Phase B delta scoped to 20.105 §3.4 + 20.101 handoff only.
