# 40.05 Master Program Guide for Thought Simulator Playground

**Document ID:** 40.05
**Version:** 0.12  
**Date:** 2026-06-08  
**Status:** **Approved** v0.12 — CuriousOne23, CP, Grok (2026-06-08)
...
| Document | Owns |
|----------|------|
| **40.05** (this guide) | Process — flows, capsule structure, evidence standards, promotion criteria |
...
## Naming Independence in the 40-Series (Standalone)

**40-series module names (.xx) are fully independent and standalone.**

You are free to choose any `40.xx` number for a new (or refactored) playground module. There is **no requirement** that a 40 number match any number used in 20, 30, 10.50, or 50.

- 20_requirements numbers are standalone.
- 50_thought_simulator_design numbers are standalone.
- Only the **30 ↔ 10.50** pair must share the same numeric band (30 follows the 10.50 anchor; 10.50 comes first as the canonical requirements layer).

When (if) work from a 40 module is promoted via 30.00, the 30 number chosen will be the one from its 10.50 peer — it may be completely different from the 40 number that supplied the evidence.

Historical modules created under the previous uniform-alignment convention (e.g. 40.50 alongside 30.220 / 10.50.220 / 50.220) may retain their numbers. New work follows the independent model.

## Controlled identity rename (40-series)

40 module renames are **address maintenance only** — no Phase B re-review when prototype logic is unchanged.

**Mandatory updates (same change set):**

- `40_name_table.json` entry (SSOT)
- `40.510_refactor.md` program row (if listed)
- `40_thought_simulator_playground/README.md` child links
- Python import paths if the module folder is renamed
- Live cross-tier citations in 30, 50, 10.50, 20 (reference sweep)

40 renames do **not** require 30 or 10.50 band changes.

Use `scripts/rename_identity.py` with `tier: "40"`. Policy: [00.00.43](../00_program_governance/00_foundations/00.00.43_controlled_identity_rename_policy.md).
...
**No-auto-propagation from 20-layer edits:** All changes driven purely from 20-series edits must obtain explicit human confirmation of selected flow direction: `forward` or `backward` before any propagation into 40, 30, or 50 artifacts.
...
2. **30-series** — When runs stabilize, evidence is normalized per [30.00](../30_verification/30.00_verification_user_guide.md) promotion standards (coverage audit; shared terms from [30.160](../30_verification/30.160_verification_glossary.md)).
...
Every 40.xx module (e.g., 40.130_gb_prototypes) **SHALL** follow this standard structure:
...
**Not evidence** (do not substitute for §8 / capsule requirements): ad hoc chat or PR comments without artifact links; screenshots or screen recordings; non-deterministic or unseeded logs; one-off manual runs not executed via `harness.py`; narrative claims with no HLR mapping in `requirements_delta.md`.

Shared vocabulary for capsule fields and artifact naming: [30.160_verification_glossary.md](../30_verification/30.160_verification_glossary.md).
...
1. Execute [40.510](40.510_refactor.md) Phase 1 — Track H intake (`40.60_*`, Class 7 replay harness).
2. Execute 40.510 Phase 2 — conversation layer + structs (USP, UPI, CIL/COB/GB redos).
3. Iterate on 40.130 Governing Basin prototype (3-tier hierarchy per 50.130; details in 40.510-206).
...
**Ownership:** 40.05 governs playground *process* (capsule structure, evidence, promotion). **[40.510](40.510_refactor.md)** governs refactor *scheduling and row sign-off*. [20.510](../20_requirements/20.510_refactoring_for_input_correction_track_h.md) tracks 20-series requirements only and has **no authority** over 40-series work.

| Priority | Suggested module / focus | 20-series inputs | Evidence target |
|----------|--------------------------|------------------|-----------------|
| P1 | `40.60_*` IIInB intake prototype — `profile_enabled` gate, read-only USP apply, TP intake-bound writes only | [20.101](../20_requirements/20.101_iiinb_requirements.md), [20.38](../20_requirements/20.38_ts_implementation_guidelines.md) §6 | `verification_capsule.md` + `requirements_delta.md` |
...
From `40.50_inb_prototypes` after a Phase B PASS — illustrative depth only:

```markdown
## Flows Alignment Statement

- **Forward Flow (20-series)**: Driven by [20.100](../20_requirements/20.100_inb_requirements.md) (non-inferential canonicalization, FIFO, isolation) and [20.38](../20_requirements/20.38_ts_implementation_guidelines.md) intake-path guards.
- **Backward Flow (40-series evidence)**: Harness 7/7 PASS (`artifacts/inb_verification_run_2026-06-07.json`) — behavioral + negative + replay for HLR-20.100-002–020; no MTP mutation demonstrated.
- **Iterative Design Flow (50-series influence)**: None yet; handoff to IIInB deferred to [40.510](40.510_refactor.md) Phase 1.

**Agreement Statement**: Provisionally aligned — InB scope is closed per 20.100; extend only for `InB → IIInB` handoff per 40.510-103, not semantic repair in InB.
```
...
Authoritative primitive definitions: [20.190_glossary.md](../20_requirements/20.190_glossary.md) (Primitive Intent Catalog). Verification terms: [30.160_verification_glossary.md](../30_verification/30.160_verification_glossary.md).
...
| 0.12 | 2026-06-08 | Grok | Light polish — not-evidence list; Appendix A filled-in example (40.50) |
| 0.12 | 2026-06-08 | CuriousOne23, CP, Grok | **Approved** — process owner sign-off |
| 0.13 | 2026-06-09 | Grok | Identity renumber — guide relocated to governance band `40.05`; module bands per [40_renumber_manifest.json](../00_program_governance/00_identity/40_renumber_manifest.json) |
