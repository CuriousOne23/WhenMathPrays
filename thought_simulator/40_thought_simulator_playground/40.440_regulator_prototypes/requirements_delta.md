...
| Explicit auditable decisions + separation (10.10.40 + 10.50.50 HLR-002) | Strongly demonstrated | explicit_auditable_decision, rich_observability | action, rationale, applied_delta, enforcement_area, obs (enforcements, boundary_marker, policy_signature) | No core mutation; rich fields |
| Input validation + rejection (10.50.50 HLR-003) | Strongly demonstrated | negative_invalid_policy, negative_negative_pressure | error on bad policy/negative pressure | Deterministic ValueError |
...
| Full deterministic mode (10.50.50 HLR-001) | Strongly demonstrated | full_deterministic_mode, positive_deterministic_replay | identical outputs + digest on replay; no nondet paths | Always det; stateful replay |
| HLR-20.450-001 (determinism/replay) | Strongly demonstrated | positive_deterministic_replay + full_deterministic_mode | verification_digest, identical decisions | Core 10.50.50 HLR |
...
| HLR-20.450-003 (input validation) | Strongly demonstrated | negative_* scenarios | ValueError on bad policy/pressure | Per 10.50.50 |
| LLR-30.50-001 (artifact emission) | Strongly demonstrated | all + harness main | full report, 15/15, per-scenario io_fields + status + digests | artifacts/regulator_verification_run_2026-06-06.json |
...
- The 12 "What Phase B Must Explore" items were each given explicit scenario coverage (plus supporting) so that 30/10.50.50/50.50 can trace specific behaviors (ΔH%, fan-out, costs, overflow, bounds, interrupts, decisions, validation, obs, bounded state, determinism).
...
- Downstream (future): 30.50_* (promotion), 10.50.50, 50.50_regulator_design_support.md (and any main 50.50 spec)
...
- Minimum regulator decision schema fields for canonical 10.50.50 audit/replay (current rich obs is strong evidence).
- Calibration/versioning of per-area limits and policy signatures (governed by 20.95/50 + 10.50.50).
...
(none this iteration; all evidence from 40 harness per 40.20. Future real testbenches under testbenches/ will feed only via 30.tb/ normalized records.)
