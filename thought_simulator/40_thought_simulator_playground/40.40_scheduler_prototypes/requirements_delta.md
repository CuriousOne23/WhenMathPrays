...
- `HLR-20.140-001`: Scheduler phase behavior shall remain deterministic and replayable for identical event sequences.
	- Evidence: `positive_deterministic_replay`
	- Traceability: `tick`, `policy`, `max_active`, `selected_tp_ids`, `history`, `verification_digest`
- `HLR-20.140-001`: Scheduler baseline fairness policy shall support deterministic round-robin progress without starvation in the exercised sequence.
	- Evidence: `positive_round_robin_fairness`
	- Traceability: `selected_tp_ids`, `wait_ticks`, `total_selected`
- `HLR-20.140-001`: Scheduler contract validation shall reject empty TP identifiers, non-monotonic ticks, and unsupported policy names.
...
- `LLR-40.40-001`: Harness shall emit a JSON artifact under `artifacts/` containing scenario outcomes and run summary.
