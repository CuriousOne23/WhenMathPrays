# 40.80_usp_prototypes / software_description.md
...
- upstream_playground_modules: [40.100](../40.100_core_data_structs_prototypes/software_description.md) (`UspRule` / `UspSnapshot` shapes), [40.60](../40.60_iiinb_prototypes/software_description.md) (consumer)
...
W2 Phase B explores the **store + snapshot + version transition** core. Full multi-turn CIL→UPI→GB wire is evidenced jointly with 40.90/40.120/40.130; USP module isolates store semantics.
...
`UspRule` is **not redefined here** — canonical leaf shape is owned by [40.100](../40.100_core_data_structs_prototypes/software_description.md) (W2 export authority; field-compatible with [40.60 `UspRule`](../40.60_iiinb_prototypes/prototype.py)). This module implements store semantics, version transitions, and snapshot assembly over imported struct shapes.
...
## Risks & Unknowns
- Simulated UPI commit API in Phase B before 40.90 lands vs minimal internal commit driver
- COB pin integration deferred to 40.110 W2 extension
...
- Blocks: 40.510-202, 40.510-204, 40.60 live USP path