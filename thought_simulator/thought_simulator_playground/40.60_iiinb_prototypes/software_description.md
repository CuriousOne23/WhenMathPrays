# 40.60_iiinb_prototypes / software_description.md
...
- Note: The full HLR list from 20.101_iiinb_requirements.md is included below for exploratory visibility. 20.xx remains the sole source of truth; 30.xx remains the authoritative coverage audit layer. 40.60 is a playground and not authoritative.
...
- cross-turn UPI/GB/CIL wiring deferred to Wave 2 (40.80, 40.90, 40.120 redos)
...
- Intake handoff contract (consumes 40.50), outbound handoff to RB, `iiinb_repair_record` skeleton
...
IIInB consumes accepted InB output per [40.50](../40.50_inb_prototypes/software_description.md) handoff contract (HLR-20.101-003, 20.100-020):
...
**Cross-turn USP version pinning:** Each repair pass loads the conversation-scoped `usp_version_ref` pinned by COB ([40.110](../40.110_cob_prototypes/software_description.md) W2 extension per 20.102-010). UPI commits on later turns advance the pin via [40.80](../40.80_usp_prototypes/software_description.md) / [40.90](../40.90_upi_prototypes/software_description.md); cross-turn replay equivalence is evidenced jointly in 40.510-204/207 Phase B (`positive_live_usp_from_40.80`).
...
**40.120 integration boundary:** `iiinb_escalation_ref` records are consumed by [40.120](../40.120_cil_prototypes/software_description.md) in deterministic FIFO per 20.032-027 after GATE-B wire (40.510-205 Phase B). W1 harness emits refs only — no live CIL callback until clarification-event wire lands.
...
- Own surface canonicalization (InB / 40.50)
- Write or commit USP rules (UPI / 40.90)
...
- Integration with live InB handoff object from 40.50 (beyond harness fixtures)
...
## Required Next Step
GATE-A closed 2026-06-08 (40.510-101). Next: W1 30-series normalize + 50 insight per [40.510](../40.510_refactor.md) §4.2.2. Deferred: HLR-024b `FAIL_ENVELOPE` negatives (40.510-207), integrated strip replay with 40.70, 30-series promotion for HLR-013/023/025 residuals.
...
- 40.50_inb_prototypes (upstream handoff evidence)
- 40.05_master_program_guide.md (workflow guidance — not treated as requirements)
- 40.510_refactor.md (W1 row 40.510-101)
- 40.60_iiinb_prototypes/prototype.py (implemented skeleton)
- 40.60_iiinb_prototypes/harness.py (19-scenario harness)
- 40.60_iiinb_prototypes/verification_capsule.md
- 40.60_iiinb_prototypes/requirements_delta.md
- 40.60_iiinb_prototypes/artifacts/iiinb_verification_run_2026-06-08.json
...
Positive guard (024a) is W1-closed (`positive_pipeline_b_envelope_unchanged`). W2 adds negative paths producing `FAIL_ENVELOPE` replay verdicts per [20.38](../../20_requirements/20.38_ts_implementation_guidelines.md) §8, exercised in [40.70](../40.70_replay_prototypes/software_description.md) strip/regen compose.
...
| `positive_live_usp_from_40.80` | 005–008 | Read-only snapshot from USP module |

### Dependencies
- [40.80_usp_prototypes](../40.80_usp_prototypes/software_description.md) (live USP snapshot path)
...
| 005–008 | Live USP snapshot path | `positive_live_usp_from_40.80` |
| 20.38 §8 | `FAIL_ENVELOPE` replay verdict | compose with [40.70](../40.70_replay_prototypes/software_description.md) strip/regen |
