# Verification Capsule — 40.100

## Status
Phase B **approved** (8/8 PASS; CP review, 2026-06-08; 40.510-203)

## Evidence
- Artifact: `artifacts/structs_verification_run_2026-06-08.json`
- Golden: `artifacts/golden_usp_snapshot_v1.json`

## Scenarios
| Scenario | Result |
|----------|--------|
| `positive_usp_snapshot_roundtrip` | PASS |
| `positive_input_repair_tag_ordering` | PASS |
| `positive_conversation_layer_envelope_clean` | PASS |
| `negative_forbidden_semantic_core_field` | PASS |
| `positive_golden_fixture_match` | PASS |
| `positive_iiinb_digest_compat` | PASS |
| `positive_audit_struct_exports` | PASS |
| `negative_unknown_schema_version` | PASS |

## HLR coverage (20.039 §3.1–3.2)
| HLR | Scenario(s) |
|-----|-------------|
| 021 | `positive_conversation_layer_envelope_clean` |
| 022 | `positive_usp_snapshot_roundtrip`, `positive_iiinb_digest_compat` |
| 024 | `positive_input_repair_tag_ordering`, `positive_golden_fixture_match`, `positive_audit_struct_exports` |
| 025 | `negative_forbidden_semantic_core_field` |
| 019 | `negative_unknown_schema_version` |