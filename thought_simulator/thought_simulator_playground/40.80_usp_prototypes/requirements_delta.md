...
| 018–019 | Replay + serialization | `positive_replay_identical_ref` (digest via 40.100) |
| 020–022 | Audit / reason codes | reason-code asserts on negative paths |
| 024 | Fixture testability | full 8-scenario matrix |

## Implemented
`USPStore.apply_commit` (create / supersede / revoke), `export_snapshot` (ACTIVE-only), monotonic `usp_version_id`, content-addressed `usp_version_ref`, GB veto path, cap overflow reject. Struct shapes and digest imported from [40.100](../40.100_core_data_structs_prototypes/prototype.py).