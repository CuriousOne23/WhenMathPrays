# 40.90_upi_prototypes / software_description.md
...
- upstream_playground_modules: [40.80](../40.80_usp_prototypes/software_description.md) (USP write surface), [40.100](../40.100_core_data_structs_prototypes/software_description.md) (`UpiCommitRecord` / event shapes), [40.120](../40.120_cil_prototypes/software_description.md) (W2 wire)
...
**GB veto reason-code propagation:** On veto, `UpiCommitRecord.commit_status = GB_VETOED`; `gb_reason_code` and `reason_codes[]` both carry the evaluator code (stub: `GB_TEST_VETO`; live: [40.130](../40.130_gb_prototypes/prototype.py) `evaluate_upi_commit` → e.g. `GB_VETO_UNSAFE_RULE`). No `usp_version_id` advance; USP remains unchanged (20.103-010, 20.102-014).
...
### USP store API boundary (40.80 dependency)

UPI owns **orchestration only** — validation, GB gate, audit emission, FIFO ordering. [40.80](../40.80_usp_prototypes/software_description.md) owns **store semantics** (version transitions, cap enforcement, `usp_version_record` append, `usp_version_ref` digest).
...
- UPI does **not** call USP snapshot export (`export_snapshot` is IIInB read-only path on 40.80)
- Wire structs (`clarification_event`, `upi_commit_record`, `UspRule`) imported from [40.100](../40.100_core_data_structs_prototypes/software_description.md); neither module redefines leaf shapes
...
- **Backward Flow (40-series evidence):** W1 [40.70](../40.70_replay_prototypes/software_description.md) C7-D/E used **simulated** UPI/GB; this module provides live commit path for W2 replay refresh.
...
**Agreement Statement**: Aligned — Phase A + Phase B approved (CP, 2026-06-08). 8/8 harness evidence; commit pipeline aligns with [20.103](../../20_requirements/20.103_upi_requirements.md) HLR-001–022. Live GB path evidenced via [40.130](../40.130_gb_prototypes/software_description.md) `harness_w2.py`. GATE-B row 202 closed.
...
## Risks & Unknowns
- GB gate simulated vs live [40.130](../40.130_gb_prototypes/software_description.md) callback in Phase B
- CIL FIFO wire timing — stub events acceptable for GATE-B isolated harness
- Cross-turn replay refresh for 40.70 C7-D/E (W2 integration bundle)
